...
class VideoEncoder(nn.Module):
    ...
    def forward(self, video_feat, video_mask=None):
        ...
        attn_mask = (video_mask == 0).to(torch.bool).to(video_feat.device) if video_mask is not None else None
...
class TextEncoder(nn.Module):
    ...
    def forward(self, text_feat, text_mask=None):
        ...
        attn_mask = (mask == 0).to(torch.bool).to(x.device) if mask is not None else None
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, x_q, x_kv, mask_q=None, mask_kv=None):
        ...
        if mask_kv is not None:
            attn_mask = (mask_kv == 0).to(torch.bool).to(x_q.device)
        x = self.ln(x_q)
...
class VideoQAModel(nn.Module):
    def __init__(self, embed_dim=2560, hidden_dim=1024, num_heads=8, num_layers=2, dropout=0.1):
        ...
        self.attn_pool = nn.MultiheadAttention(hidden_dim, num_heads, batch_first=True)
    ...
    def _pool_with_cls(self, seq, cls_token):
        BxC = seq.size(0)
        cls = cls_token.expand(BxC, -1, -1).to(seq.device)  # (B*C, 1, D)
        seq_with_cls = torch.cat([cls, seq], dim=1)         # (B*C, 1+S, D)
        q = seq_with_cls[:, :1, :]                          # (B*C, 1, D)
        out, _ = self.attn_pool(q, seq_with_cls, seq_with_cls)  # (B*C, 1, D)
    ...
    def forward(self, video_feat, text_feat, video_mask=None, text_mask=None):
        ...
        t_mask = text_mask.view(B * C, T_text).to(torch.bool).to(text_seq.device) if text_mask is not None else None
        v_mask = video_mask.unsqueeze(1).repeat(1, C, 1).view(B * C, -1).to(torch.bool).to(v_seq.device) if video_mask is not None else None
        ...
        v_cross = self.cross_attn_v2t(v_seq, text_seq, mask_q=v_mask, mask_kv=t_mask)  # (B*C, T_v, H)
        t_cross = self.cross_attn_t2v(text_seq, v_seq, mask_q=t_mask, mask_kv=v_mask)  # (B*C, T_text, H)

        v_pooled = self._pool_with_cls(v_cross, self.cls_token).view(B, C, -1)  # (B, C, H)
        t_pooled = self._pool_with_cls(t_cross, self.cls_token).view(B, C, -1)  # (B, C, H)
